...
static void addAttach(Camera*, Box*);
...
static void addAttach(Camera* this, Box* box)
{
...
static void jump(Camera* this)
{
    ...
    if (start)
    {
        ...
        {
            attach = (Box*)(node->value);
            attach->recordInitialHeight(attach);
        }

        return;
    }

    posDelta = calcJump(glfwGetTime() - startTime);
    ...
    {
        this->position[1] += posDelta - lastValue;
        glm_vec3_copy((vec3){0.0f, posDelta - lastValue, 0.0f}, temp);
        lastValue = posDelta;
